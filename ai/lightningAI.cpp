...
Result LightningAI::run(){
    ...
    //game.board.expectedScore = calc(field, 0);
    ...
    for (i = 0; i < game.units.size(); i++) {
        ...
        for (j = 0; j < beamWidth && !que.empty(); j++) {
            ...
            if (!Util::check(board.field, game.units[i].pivot, 0, game.units[i])) continue;
            ...
            while (!queBFS.empty()) {
                ...
                for (k = 0; k < 4; k++) {
					...
					if (Util::check(board.field, nextPoint, theta, game.units[i])) {
                ...
                for (k = -1; k <= 1; k++) {
                    ...
                    if (Util::check(board.field, point, nextTheta, game.units[i])) {
