...
class BitRow {
    ...
    inline bool check(int width) const {
        if (bits != (1ULL << width) - 1) return false;
...
class Board {
    ...
    unsigned long long commands[256];
    int commandLength;
    vector <BitRow> field;
    ...
    void addCommand(char command) {
        ...
        commands[commandLength / 21] |= c << ((commandLength % 21) * 3);
        commandLength++;
    }
    ...
    string getCommands()const {
        ...
        for (int i = 0; i < commandLength; i++) {
            char c = (char) ((commands[i / 21] >> ((i % 21) * 3)) & 7);
...
int main()
{
    ...
    memset(initBoard.commands, 0, sizeof(initBoard.commands));
            ...
            while (!queBFS.empty()) {
                ...
                for (k = 0; k < 4; k++) {
                    ...
                    if (check(board.field, nextPoint, theta, i)) {
                        ...
                        nextBoard.addCommands(commands);
